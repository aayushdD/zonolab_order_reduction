...
for iter = 1:iterMax
    ...
    if isempty(zeroCols) && isempty(zerorows)
        ...
        for m = 1:ng
            ...
            prob_base = struct();
            prob_base.P = sparse(size(G, 2), size(G, 2)); % Zero quadratic term
            prob_base.A = sparse([A; eye(size(G, 2))]); % Combine equality and bound constraints
            ...
            %% Upper bound check (x_j ≥ 1 + tolFeas)
            prob = prob_base;
            prob.q = s; % Original objective
            lbs(j) = 1 + tolFeas;
            ubs(j) = inf;
            prob.l = [b; lbs];
            prob.u = [b; ubs];
            ...
            solver = osqp;
            solver.setup(prob.P, prob.q, prob.A, prob.l, prob.u, settings);
            result = solver.solve();

             RjLPub = result.info.status;

             if strcmp(RjLPub, 'solved')
                RjLPlb = RjLPub;  % If the solution is optimal, store the result status
             else

            %% Lower bound check (x_j ≤ -1 - tolFeas)
                prob = prob_base;
                prob.q = -s; % Negated objective for second check
                ubs(j) = -1 - tolFeas;
                lbs(j) = -inf;
                prob.l = [b; lbs];
                prob.u = [b; ubs];
                ...
                solver.setup(prob.P, prob.q, prob.A, prob.l, prob.u, settings);
                result = solver.solve();
                RjLPlb = result.info.status;
             end

            redundant = (~strcmp(RjLPub,'solved')) && (~strcmp(RjLPlb,'solved'));
