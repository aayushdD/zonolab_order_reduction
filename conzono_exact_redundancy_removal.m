function Zi = conzono_exact_redundancy_removal(Zi)
    ...
    count = 0;
    iterMax = 1000000; % Max iterations
    for iter = 1:iterMax
        ...
        if isempty(zeroCols) && isempty(zerorows)
            ...
            for m = 1:ng
                ...
                if strcmp(RjLPub, 'OPTIMAL')
                    % Would switching the order of ub vs lb reduce number of gurobi calls?
                    RjLPlb = RjLPub;
                else
                    lbs(j) = -inf;
                    ubs(j) = -1 - tolFeas;
